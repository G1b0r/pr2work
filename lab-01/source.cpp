...
int main() 
{
	...
	for (i = 0; i < 15 && fajl.eof(); i++) 
	{
		...
		do {
			...
			if (tmb[i].suly < 95 || tmb[i].suly >150) {
				cout << "Hibas adat, a sulynak 95 es 150 kozott kell lennie" << endl;
			}
	...
	for (i = 0; i < letszam; i++) {
		cout << setw(20) << tmb[i].nev << "\t rajtszam: " << tmb[i].rajtszam << "\t suly: " << tmb[i].suly << "\t szakitas: " << tmb[i].szakitas << "\t lokes: " << tmb[i].lokes << "\t osszesitett: " << tmb[i].lokes + tmb[i].szakitas << endl;
